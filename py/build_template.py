...
PREFILL_FROM_IMAGE = [
    ...
    #   최종인(09:00~12:00 고정, 26.8.20~), 이성돈 연속게임=금지(26.8.20~).
    #   경성현: 26.8.20~ 09:00~12:00이었으나 **26.9.10부터 07:00~12:00으로 환원**
    #   (사용자 지시 "경성현 씨는 계속 7시부터 12시까지로. 이번주만이 아니고").
    # 신혁재·방미라 부부 '종료시각 반드시 30분 차이'는 멤버 설정(COUPLES_DEFAULT 종료시간차=30)이 담당.
    # (이름, 성별, 구력, 구분, IN, OUT, 최소게임수, 최대게임수, 연속게임, 채움, 메모)  단일 클럽이라 클럽 칸은 비워 둠.
    # 채움(26.9.10~): 김도윤(클럽 회장)만 '예' - 남는 자리 채움 역할.
    #   최소게임수는 두지 않는다(사용자 확정 26.9.10: "0으로 되도 돼", "최소게임수 만들지마").
    ("경성현", "남", 4,  "정회원", "07:00", "12:00", "", "", "", "", ""),
    ...
    ("김도윤", "남", 10, "정회원", "07:00", "12:00", "", "", "", "예", "회장 - 남는 자리 채움"),
...
def _build_players_sheet(ws, prefill: str = ""):
    headers = ["번호", "이름", "성별", "구력", "구분", "클럽", "IN시간", "OUT시간",
               "최소게임수", "최대게임수", "혼복희망", "연속게임", "채움", "메모"]
    widths = [6, 12, 8, 8, 12, 12, 10, 10, 12, 12, 10, 10, 10, 28]
    ...
    for r in range(2, 32):
        ...
        if idx < len(prefill_rows):
            name, gender, exp, mem, in_t, out_t, min_g, max_g, streak, fill, memo = prefill_rows[idx]
            ...
            if fill:
                ws.cell(row=r, column=13, value=fill)
                ws.cell(row=r, column=13).fill = PREFILL_NOTE_FILL
            if memo:
                ws.cell(row=r, column=14, value=memo)
                ws.cell(row=r, column=14).fill = PREFILL_NOTE_FILL
    ...
    dv_fill = DataValidation(type="list", formula1='"예"', allow_blank=True)
    dv_fill.add("M2:M31")
    ws.add_data_validation(dv_fill)
...
def _build_guide_sheet(ws):
    ...
    lines = [
        ...
        ("• 채움: `예`라고 적으면 그 사람은 다른 사람 배정이 끝난 뒤 남는 자리에만 들어갑니다", False),
        ("    - 최소게임수 칸에 적은 만큼은 보장됩니다. 비워 두면 일반 참가자입니다 (선택)", False),
