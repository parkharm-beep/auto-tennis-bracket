...
def _build_guide_sheet(ws):
    ...
    lines = [
        ...
        ("  안 나올 때입니다. 쓸 만하다 = ① 같은 짝이 처음이고 ② 같은 4명이 다시", False),
        ("  붙는 게 아니고 ③ 두 팀 구력 합 차이가 4 이내.", False),
        ("    예) 여자 4명 구력이 3/3/5/7이면 편 가르는 방법이 3가지인데", False),
        ("        (3+3 vs 5+7)은 구력 합 6 차이라 재미가 없습니다 → 여자복식은 2판이 한계.", False),
        ("        모자란 게임수는 혼합복식으로 채웁니다.", False),
        ...
        ("• 합산 구력 차이가 4를 넘으면 강하게 회피 — 그렇게 될 바에는 다른 조합이나", False),
        ("  혼합복식으로 돌립니다 (억지로 짜서 재미없는 경기가 되지 않도록)", False),
