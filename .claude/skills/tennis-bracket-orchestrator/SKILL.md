...
- **그 밖에 혼복이 늘어나는 경우** — 남복/여복을 더 짜봐야 **'쓸 만한 대진'이 안 나올 때**.
  쓸 만하다 = ① 같은 짝이 처음 ② 같은 4명이 다시 만나는 게 아님 ③ **두 팀 구력합 차이 ≤ 4**.
...
- **상한 계산.** `good_singles_capacity()`가 위 세 조건을 만족하는 단성 복식을 최대 몇 판
  ...
  - 예: 여자 4명 구력 3/3/5/7 → 편 가르는 법 3가지 중 (3+3 vs 5+7)은 구력차 6이라 탈락
    → **쓸 만한 여복은 2판이 한계** → 혼복 상한 4판
  - 예: 여자 5명 구력 3/3/5/5/7 → 5판까지 가능 → 혼복은 기본 보장 1~2판만
...
- **경기 사이 1시간 이상 공백을 최우선 회피** (`long_gap=150 × (초과 30분 단위)²`).
