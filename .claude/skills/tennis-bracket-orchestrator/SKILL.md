...
- **그 밖에 혼복이 늘어나는 경우** — 다음 두 가지뿐:
  1. 남자(또는 여자)가 4명 미만이라 단성 복식 자체가 불가능 (상한 무제한)
  2. 단성 복식을 더 짜면 **같은 4명이 상대편까지 그대로** 다시 붙게 될 때
- **상한 계산.** 여자가 정확히 4명이면 편 가르는 방법이 3가지뿐이라 **여복은 3판이 한계**다.
  1인당 공평 게임수가 4판이면 1판이 모자라므로 `ceil(4명 × 1판 / 2) = 2판`까지 올린다.
  초과분은 `mixed_over_quota`로 급격히 비싸진다. 교류전도 같은 규칙(팀 구성 가능 여부는 별도 확인).
