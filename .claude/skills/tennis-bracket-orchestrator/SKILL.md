...
- **혼복은 차선책** — 다음 두 경우에만 나온다:
  ...
  2. 인원이 적어 **같은 페어가 반복될 수밖에 없을 때** (짝 바꾸기용 완충)
- 구현: `single_mixed_nonpriority=70`(단일 클럽) / `mixed_nonpriority=120`(교류전) —
  단성 복식이 가능한 상황에서 혼복을 고를 때만 부과. 전체 점수에도 `mixed_match=45`/경기.
  페어 중복 비용은 초선형(`30*(count-1)²`)이라, 같은 짝이 3~4번 반복될 상황이면 혼복이 열린다.
