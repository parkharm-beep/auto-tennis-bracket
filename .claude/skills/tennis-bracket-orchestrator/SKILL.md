...
- **혼복은 차선책** — 다음 두 경우에만 나온다:
  1. 남자(또는 여자)가 4명 미만이라 단성 복식 자체가 불가능
  2. 단성 복식을 더 짜면 **같은 4명이 상대편까지 그대로** 다시 붙게 될 때
- **허용량(`mixed_quota`)만큼만 연다.** 여자가 정확히 4명이면 편 가르는 방법이 3가지뿐이라
  **여복은 3판이 한계**다. 1인당 공평 게임수가 4판이면 1판이 모자라므로
  `ceil(4명 × 1판 / 2) = 혼복 2판`을 허용한다. 초과분은 `mixed_over_quota`로 급격히 비싸진다.
  교류전은 (클럽,성별) 사정이 제각각이라 고정 2판.
