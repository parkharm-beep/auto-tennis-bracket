...
- **`mixed_quota`** — 허용 혼복 경기 수. 소수 성별이 정확히 4~5명이면 중복 없는 단성 복식을
  몇 판밖에 못 만들므로(여자 4명 → 편 가르는 법 3가지 → 여복 3판이 한계), 모자란 게임수만큼만 계산한다.
  `ceil(소수성별인원 × (1인당 공평 게임수 - 중복없는 단성복식 최대판수) / 2)`.
  21명·20경기·여자 4명이면 **2판**. 소수 성별 6명 이상이면 0, 4명 미만이면 무제한(단성 복식 불가).
  교류전은 (클럽,성별) 사정이 제각각이라 고정 `EXCHANGE_MIXED_QUOTA=2`.
