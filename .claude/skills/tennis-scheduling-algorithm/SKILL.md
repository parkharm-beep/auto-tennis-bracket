...
- **같은 4명 재대결** — `quad_repeat`(편만 바꿔 재대결, 약함) / `matchup_repeat`(상대편까지 그대로, 강함)
...
- **`mixed_limits()`** — 혼복 판수의 **(최소 보장, 허용 상한)**.
  - **여자 6명 이하 → 최소 1판 / 상한 2판** (`MIXED_SMALL_WOMEN`/`MIXED_MIN_GAMES`/`MIXED_DEFAULT_QUOTA`).
    여복만 돌리면 늘 같은 사람끼리 붙게 되므로 기본적으로 1~2판을 넣는다.
  - 소수 성별이 4~5명이면 중복 없는 단성 복식을 몇 판밖에 못 만들므로
    (여자 4명 → 편 가르는 법 3가지 → 여복 3판이 한계) 모자란 게임수만큼 **상한을 더 올린다**:
    `ceil(소수성별인원 × (1인당 공평 게임수 − 중복없는 단성복식 최대판수) / 2)`
