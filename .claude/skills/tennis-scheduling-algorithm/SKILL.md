...
- **같은 4명 재대결** — `quad_repeat=60`(편만 바꿔 재대결) / `matchup_repeat=90`(상대편까지 그대로)
...
- **'쓸 만한 단성 복식'의 정의 (`clean_singles_available`)** — 다음을 모두 만족해야 한다:
  ① 같은 짝이 처음 ② 같은 4명이 다시 만나는 게 아님(`quad_count`) ③ 두 팀 구력합 차이 ≤ `SINGLES_SKILL_TOL=4`.
...
- **`good_singles_capacity()`** — 짝도 안 겹치고 구력도 맞는 단성 복식을 최대 몇 판 만들 수 있는지.
  ...
  예: 여자 4명 구력 3/3/5/7 → 편 가르는 법 3가지 중 (3+3 vs 5+7)은 구력차 6이라 탈락 → **2판이 한계**.
- **`mixed_limits()`** — 혼복 판수의 **(최소 보장, 허용 상한)**.
  ...
  - 쓸 만한 단성 복식이 모자라면 그만큼 **상한을 더 올린다**:
    ...
    → 여자 4명 3/3/5/7·20경기면 상한 4판, 여자 5명 3/3/5/5/7이면 상한 2판.
...
- **같은 4명 재대결** — `quad_repeat=130/회`(편만 바꿔 재대결), `matchup_repeat=220*(초과)²`(상대편까지 그대로)
- **혼복 개수** — 허용량 이내는 `mixed_match=20/경기`, 초과분은 `mixed_over_quota=220*(초과)²`,
  **최소 보장 미달분은 `mixed_below_min=300*(부족)²`** (그리디에서도 미달 동안은 혼복을 우대)
- **구력차** — `team_skill_diff=4/년`에 더해, 차이가 `SINGLES_SKILL_TOL=4`를 넘으면
...
- **여자가 정확히 4명이면 여복은 3판이 한계** (편 가르는 법이 3가지뿐).
  그 이상 뛰려면 완전 중복 대진이 되므로 부족분은 혼복 1~2판으로 채운다.
